    ...
    return async (req, res, next) => {
      ...
      if (fileToGet.includes(".json")) {
        try {
          ...
          streamToString(fileStream)
            .then((data) => {
              res.end(data);
            })
            .catch((err) => {
              if (_.get(err, "code") === 404) {
                storageLogger.s404(res, "GCLOUD__StorageService : readStream error - Error " + _.get(err, "code") + " " + _.get(err, "message"), "", _.get(err, "message"));
              } else {
        ...
        this.fileExists(bucketName, fileToGet, "", (error, fileExists) => {
          if (error) {
            ...
            this.getSharedAccessSignature(bucketName, fileToGet, "", undefined, (err, presignedURL) => {
              if (err) {
                storageLogger.s500(res, "GCLOUD__StorageService : readStream_getSharedAccessSignature - Error 500. Failed to get shared access signature", err, err);
              } else {
                const response = {
                  responseCode: "OK",
                  params: {
            ...
            storageLogger.s500(res, "GCLOUD__StorageService : readStream_fileExists error - Error 500. Failed to fetch or File does not exists", error, "Failed to fetch or File does not exists");
          }
        });
      }
    };
  ...
  async getBlobProperties(request, callback) {
    ...
    file.getMetadata((err, metadata, resp) => {
      if (err) {
        ...
        logger.info({
          msg: "GCLOUD__StorageService : getBlobProperties_getMetadata success with status code 200. File exists - " + request.file,
          statusCode: _.get(resp, "statusCode"),
        });
